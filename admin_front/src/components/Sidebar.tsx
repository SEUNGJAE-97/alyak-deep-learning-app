...
function ResourceItem({ icon: Icon, label, detail, usage }: { icon: any, label: string, detail: string, usage: number }) {
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-end">
        <div className="flex items-center gap-2">
          <Icon className="w-3 h-3 text-on-surface-variant opacity-50" />
          <span className="text-[9px] font-bold text-on-surface uppercase tracking-wider">{label}</span>
        </div>
        <span className="text-[8px] font-mono text-on-surface-variant">{detail}</span>
      </div>
      <div className="relative h-1 bg-surface-container-highest rounded-full overflow-hidden">
        <motion.div 
          ...
          className={cn(
            ...
            usage > 80 ? "bg-error" : usage > 50 ? "bg-tertiary" : "bg-primary shadow-[0_0_8px_rgba(123,208,255,0.4)]"
          )}
        />
      </div>
    </div>
...
export default function Sidebar({ onLogout, currentView, onViewChange }: SidebarProps) {
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 z-40 bg-surface-container-lowest border-r border-outline-variant/10 flex flex-col p-4 gap-2">
      <div className="mt-20 mb-8 px-2 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-sm font-black text-primary leading-none uppercase tracking-tighter">Pill RL Engine</h2>
          <p className="text-[10px] text-on-surface font-black uppercase tracking-widest mt-1 bg-primary/10 px-1.5 py-0.5 rounded border border-primary/20 inline-block">v2.1.0-ltyk</p>
        </div>
      </div>
      ...
      <div className="px-2 mb-6 space-y-4">
        <div className="bg-surface-container-low/40 backdrop-blur-xl border border-outline-variant/10 rounded-2xl p-4 shadow-xl">
          ...
          <div className="space-y-3">
            <ResourceItem icon={Cpu} label="CPU" detail="EPYC 7763" usage={42} />
            <ResourceItem icon={Database} label="RAM" detail="512GB DDR4" usage={68} />
            <div className="relative group">
              <ResourceItem icon={ShieldCheck} label="GPU" detail="H100 x 4" usage={85} />
