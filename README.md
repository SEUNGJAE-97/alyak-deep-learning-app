...
현재 저장소에는 사용자용 Android 앱뿐 아니라 백엔드 API, OCR 서비스, 관리자용 학습 대시보드, 모델 실험 산출물까지 함께 포함되어 있습니다.
...
| 경로 | 역할 | 주요 스택 |
|------|------|-----------|
| [`application_project/`](./application_project/) | 사용자용 Android 앱 | Kotlin, Jetpack Compose, Hilt, Room, Retrofit, FCM |
| [`alyak-api-server/`](./alyak-api-server/) | 메인 API 서버, 인증, 가족/복약/약품 도메인, 관리자 API | Spring Boot 3, Java 17, JPA, MySQL, Redis, Firebase Admin |
| [`Pill-Recognition-Pipeline/`](./Pill-Recognition-Pipeline/) | 알약 이미지 OCR 및 추론용 Python 서비스 | FastAPI, EasyOCR, OpenCV, Ultralytics |
| [`admin_front/`](./admin_front/) | 관리자용 웹 대시보드 | React, TypeScript, Vite |
| [`deep_learning_prj/`](./deep_learning_prj/) | 모델 가중치, 추론 결과, 실험 노트북 등 연구용 산출물 | PyTorch/YOLO 계열 산출물 포함 |
| [`shared-images/`](./shared-images/) | 서비스 간 공용 이미지 자산 저장 용도 디렉터리 | 정적 파일 저장 |
...
현재 이 디렉터리에는 별도 `README.md`가 없으므로, 실제 실행 전에는 `requirements.txt`, `Dockerfile`, 앱 엔트리 파일을 함께 확인하는 것이 좋습니다.
...
이 디렉터리는 제품 서비스 코드보다는 연구/실험 자산에 가깝습니다.

## 실행 전 설정 가이드

이 저장소는 단순히 `docker compose up` 또는 `npm run dev`만으로 바로 실행되는 구조가 아닙니다.  
서브프로젝트별로 환경 변수, 비밀 키, 외부 서비스 설정 파일이 필요하므로 먼저 아래 항목을 준비하는 것을 권장합니다.

### 공통 원칙

- 민감한 값은 Git에 커밋하지 말고 로컬 전용 파일로 관리하세요.
- 경로 기반 비밀 파일은 팀 내에서 위치 규칙을 정한 뒤 동일하게 맞추는 편이 좋습니다.
- 루트 문서는 "무엇을 준비해야 하는지"를 안내하고, 실제 값은 각자 발급받아 넣는 방식으로 운영하는 것을 권장합니다.
...
백엔드와 OCR/학습 서비스는 `shared-images` 디렉터리를 공유해서 사용합니다.
...
### OCR / Python 서비스 (`Pill-Recognition-Pipeline`) 준비 사항

Python 서비스는 단독 OCR 서버이기도 하고, 관리자 학습 파이프라인의 내부 서비스 역할도 합니다.
...
`admin_front`는 기본적으로 Spring 관리자 API를 바라보며, 일부 템플릿 잔여 설정이 남아 있습니다.
...
또한 `admin_front/.env.example`에는 `GEMINI_API_KEY`가 남아 있는데, 현재 관리자 기능의 핵심 흐름은 Spring API 연동이므로 이 값이 반드시 필요한지 여부는 별도 확인 후 정리하는 것이 좋습니다.
...
- `application_project/app/google-services.json`
- `application_project/local.properties` 또는 이에 준하는 로컬 키 설정
- `admin_front/.env.local`

팀 온보딩 문서처럼 관리하려면 위 파일들의 예시 템플릿을 별도로 만드는 것도 추천합니다.
...
4. `admin_front/.env.example`와 실제 코드의 `VITE_API_BASE_URL` 사용 위치 확인
...
- [관리자 웹 기본 README](./admin_front/README.md)

## 참고

- 현재 루트 README는 실제 저장소 구조를 기준으로 갱신되었습니다.
- `admin_front/README.md`는 아직 템플릿 성격의 기본 문서라, 필요하면 다음 단계로 별도 정리할 수 있습니다.
